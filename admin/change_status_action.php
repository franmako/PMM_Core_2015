...
if (!empty($_SESSION) AND $_SESSION['userlevel'] == USER_ADMIN) {
	...
	if (hash_equals($hash_db, $hash)){
		$query_status= "SELECT * FROM user_status WHERE users_id = $userID_user;";
		...
		switch ($status_new) {
			case 1:
				$query_update_db= "UPDATE user_status SET level=$status_new,label_level='Sous-Admin' WHERE id=$status_id";
				$update_status= $db_connect->query($query_update_db);
				break;
			case 2:
				$query_update_db= "UPDATE user_status SET level=$status_new,label_level='Normal' WHERE id=$status_id";
				$update_status= $db_connect->query($query_update_db);
				break;
			case 6:
				$query_update_db= "UPDATE user_status SET level=$status_new,label_level='Gelé' WHERE id=$status_id";
				$update_status= $db_connect->query($query_update_db);
				break;
			case 7:
				$query_update_db= "UPDATE user_status SET level=$status_new,label_level='Dèsinscrit' WHERE id=$status_id";
				$update_status= $db_connect->query($query_update_db);
				break;
			case 8:
				$query_update_db= "UPDATE user_status SET level=$status_new,label_level='Banni' WHERE id=$status_id";
				$update_status= $db_connect->query($query_update_db);
				break;
		}
