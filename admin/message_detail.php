...
if ($message_read == 1) {
	$query_read= "UPDATE contact_messages SET message_read=1 WHERE id= $messageID";
	...
	$query_unread= "UPDATE contact_messages SET message_read=0 WHERE id= $messageID";
	$db_connect= db_connect();
...
if($parent_id != NULL){
	$query_parent= "SELECT * FROM contact_messages WHERE id=$parent_id";
}
