...
function user_info_display(){
	...
	$query_secret="SELECT * FROM users_noyau WHERE id=$userID";
	$row_secret = getRow($query_secret);
	if ($row_secret) {
		$secretquestion= $row_secret['secret_question'];
	} else {
		echo "Erreur!";
	}

	if($secretquestion == 0){
		echo '<p><a href="index.php?rq=secretQuestion_set">[Cliquez ici pour ajouter une question secrète.]</a></p>';
	}else {
		echo '<p><a href="index.php?rq=secretQuestion_modify">[Modifier - Réponse Secrète]</a></p>';
	}
} 
?>